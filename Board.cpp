...
void Board::Update(std::vector<std::vector<LIBCARD::Card> >& _display) const
{
    ...
    _display.clear();
    for (size_t i = 0; i < FREE_CELL_SIZE; ++i)
    {
    ...
    for (size_t i = 0; i < HOME_CELL_SIZE; ++i)
    {
        std::vector<LIBCARD::Card> v;
        if (m_homecell.Size(i) > 0)
        {
            size_t last_pos = m_homecell.Size(i) - 1;
            v.push_back(m_homecell.Get(i, last_pos));
        }
        else
        {
            v.push_back(LIBCARD::Card(LIBCARD::CardSuit::NOTHING, LIBCARD::CardRank::NOTHING));
